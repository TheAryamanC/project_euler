"""
Project Euler - Problem 65: Convergents of e

The square root of 2 can be written as an infinite continued fraction: sqrt(2) = 1 + 1/(2 + 1/(2 + 1/(2 + ...)))
The infinite continued fraction can be written, sqrt(2) = [1; (2)], (2) indicates that 2 repeats ad infinitum. 
In a similar way, sqrt(23) = [4; (1, 3, 1, 8)].

It turns out that the sequence of partial values of continued fractions for square roots provides the best rational approximations.
Let us consider the convergents for sqrt(2).
The first ten convergents for sqrt(2) are:
1, 3/2, 7/5, 17/12, 41/29, 99/70, 239/169, 577/408, 1393/985, 3363/2378, ...

The continued fraction for e is: e = [2; 1,2,1, 1,4,1, 1,6,1, ... ,1,2k,1,...]

The first ten terms in the sequence of convergents for e are:
2, 3, 8/3, 11/4, 19/7, 87/32, 106/39, 193/71, 1264/465, 1457/536, ...

The sum of digits in the numerator of the 10th convergent is 1+4+5+7 = 17.

Find the sum of digits in the numerator of the 100th convergent of the
continued fraction for e.
"""

def solution():
    def e_coefficient(i):
        if i == 0:
            return 2
        if i % 3 == 2:
            return 2 * (i // 3 + 1)
        return 1
    
    n_prev, n_curr = 1, 2
    
    for i in range(1, 100):
        a = e_coefficient(i)
        n_prev, n_curr = n_curr, a * n_curr + n_prev
    
    return sum(int(d) for d in str(n_curr))

if __name__ == "__main__":
    print(solution())  # Answer: 272
